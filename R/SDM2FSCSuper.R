#' @title 2-Span Slope Deflection Method(Fixed-Support-Cantilever) with Multiple Load
#'
#' @description This function computes the moments and the rotations using slope deflection method(SDM).
#'
#' @param InputMatrix The matrix with parameter values for SDM2FSC().
#'
#' @export
...
SDM2FSCSuper=function(InputMatrix){
  FEMABG=0
  FEMBAG=0
  FEMBCG=0
  FEMCBG=0
  ...
  for(i in 1:dim(InputMatrix)[1]){
    x=SDM2FSC2(as.numeric(InputMatrix[i,1]),
  ...
  print(c("Theta C",ThetaCG))

}


SDM2FSC2=function(LengthAB,
                 LengthBC,
                 Typeof1,
                 Typeof2,
                 Magnitude1,
